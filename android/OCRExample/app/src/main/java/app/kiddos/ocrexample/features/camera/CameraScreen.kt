...
private fun CameraContent(
    ...
    var detectedTexts by remember { mutableStateOf<List<DetectedText>>(emptyList()) }
    ...
    Scaffold(
        ...
        floatingActionButton = {
            ExtendedFloatingActionButton(
                ...
                onClick = { capturePhoto(context, cameraController, ocrManager, onPhotoCaptured, { detectedTexts = it }) },
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            ...
            if (lastCapturedPhoto != null) {
                ...
                TextOverlay(
                    ...
                    sourceBitmap = lastCapturedPhoto,
                    modifier = Modifier.fillMaxSize()
                )
            }
...
private fun capturePhoto(
    ...
    onPhotoCaptured: (Bitmap) -> Unit,
    onResult: (List<DetectedText>) -> Unit,
) {
    ...
    cameraController.takePicture(mainExecutor, object : ImageCapture.OnImageCapturedCallback() {
        ...
        override fun onCaptureSuccess(image: ImageProxy) {
            ...
            if (img != null) {
                ...
                ocrManager.runTextRecognition(inputImage, onResult = onResult)
            }
...
fun TextOverlay(
    ...
    sourceBitmap: Bitmap?, // Pass the actual last captured photo
    modifier: Modifier = Modifier
) {
    if (sourceBitmap == null || detectedTexts.isEmpty()) return
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        val scaleX = size.width / sourceBitmap.width.toFloat()
        val scaleY = size.height / sourceBitmap.height.toFloat()

        detectedTexts.forEach { detection ->
            val rect = detection.boundingBox
            val scaledLeft = rect.left * scaleX
            val scaledTop = rect.top * scaleY
            val scaledWidth = (rect.right - rect.left) * scaleX
            val scaledHeight = (rect.bottom - rect.top) * scaleY
            ...
            if (detection.text.isNotBlank()) {
                drawText(
                    ...
                    text = detection.text,
                    topLeft = Offset(scaledLeft, scaledTop - 20.dp.toPx()), // Positioned slightly above
                    style = style
                )
            }
        }
